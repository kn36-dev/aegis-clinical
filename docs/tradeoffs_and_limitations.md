...
Workflow checkpoints intentionally store only the minimum state required for
deterministic execution resumption rather than complete telemetry, keeping workflow
persistence and observability as separate concerns. **v1 observability** is provided by
workflow-state visibility: the `GET /api/v1/workflows/{workflow_id}` endpoint and the
React workflow-stage timeline / review queue expose where a case is in the pipeline.

> **Future v2:** OpenTelemetry is **not implemented** today. A production enhancement
> would add distributed tracing, span-level visibility, and token/latency telemetry,
> exported via OTLP to a dedicated backend such as Jaeger, Grafana Tempo, or OpenObserve —
> with checkpoints carrying trace-correlation identifiers rather than full spans. This is
> roadmap, not a current claim of end-to-end instrumentation.
...
"Local mode" was originally exercised only by the evaluation harness's fixture index (`aegis.evaluation.local_index`). The `AEGIS_PROFILE=demo-local` runtime profile is the same abstraction (`LocalVectorStore`/`LocalVectorQueryProvider`) applied at request-serving time, compiled from the real ICD-11 taxonomy rather than a small fixture — see `docs/adr/0003-local-demo-execution-strategy.md` for why that compile is cached instead of repeated on every start.
...
This is documented rather than patched, because the available fixes are both architectural changes outside this slice's scope: relaxing the content store's referential integrity to the patient case, or making `case_id` supplied by the caller rather than generated by `ClinicalNoteService`. `tests/integration/test_clinical_pipeline.py` sidesteps this by substituting a fake content repository that resolves any reference from an in-memory mapping with no case-identity dependency, which is sufficient to exercise the full workflow, interrupt/resume, and persistence-then-cache ordering end to end — just not the live content-seeding path itself, which remains a known gap for a future slice to close.

The demo, demo-local, and integration profiles (`AEGIS_PROFILE=demo`/`demo-local`/`integration`, `aegis.api.bootstrap.build_content_repository`) are further consumers of this same workaround, not a new gap: all three substitute the identical in-memory `FakeContentRepository`, pre-seeded with a fixed set of sample notes (`aegis.api.bootstrap.DEMO_SAMPLE_NOTES`), for exactly the reason above. Content resolution is the one collaborator all three fake regardless of how real everything else is — a fresh submission against the real `SQLiteContentStore` fails the same way no matter which other collaborators are real, up to and including `demo-local`, whose vector retrieval is also a substitute (a locally compiled index rather than Upstash Vector, see `docs/adr/0003-local-demo-execution-strategy.md`) but whose embedding, cache, reasoning, and content boundaries otherwise mirror `demo`'s.
