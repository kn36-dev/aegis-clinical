...
The demo and integration profiles (`AEGIS_PROFILE=demo`/`integration`, `aegis.api.bootstrap.build_content_repository`) are further consumers of this same workaround, not a new gap: both substitute the identical in-memory `FakeContentRepository`, pre-seeded with a fixed set of sample notes (`aegis.api.bootstrap.DEMO_SAMPLE_NOTES`), for exactly the reason above. Unlike the fake-adapter test path, neither profile is credential-free otherwise — both keep real embedding and real Upstash Vector retrieval, and integration additionally uses the real Redis cache and real CrewAI/Groq reasoning (`scripts/demo_e2e.py`, `scripts/integration_e2e.py`, `scripts/e2e_common.py`); content resolution is the one collaborator both fake regardless, since a fresh submission against the real `SQLiteContentStore` fails the same way no matter which other collaborators are real.
