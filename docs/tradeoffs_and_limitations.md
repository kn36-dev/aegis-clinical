...
The demo profile (`AEGIS_PROFILE=demo`, `aegis.api.bootstrap.build_content_repository`) is a second consumer of this same workaround, not a new gap: it substitutes the identical in-memory `FakeContentRepository`, pre-seeded with a fixed set of sample notes (`aegis.api.bootstrap.DEMO_SAMPLE_NOTES`), for exactly the reason above — a fresh submission against the real `SQLiteContentStore` fails the same way regardless of which other collaborators (cache, reasoning) are real or fake in a given profile.
