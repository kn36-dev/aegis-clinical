...
into ``patient_identity_vault`` for ``AEGIS_PROFILE`` in ``("demo",
"demo-local")`` -- the same real system-of-record table and column set
every other patient identity lives in, not a separate store.
"""
...
def list_demo_patients() -> list[DemoPatientResponse]:
    ...
    Returns an empty list -- not a 404 -- whenever ``AEGIS_PROFILE`` is
    not ``"demo"`` or ``"demo-local"``: this route is a permanent part
    of the API contract, only its dataset is profile-scoped. A caller
    (the physician submission UI) always gets a list back and decides
    how to render zero results; it never needs to special-case "this
    endpoint doesn't exist here".
    """
