...
@patch(f"{MODULE}.build_clinical_reasoning_crew")
@patch(f"{MODULE}.LLM")
async def test_reason_builds_crew_with_llm_and_prompt_then_kicks_off(mock_llm_cls, mock_build_crew):
    mock_llm = MagicMock()
    ...
    mock_crew.kickoff_async = AsyncMock(
        return_value=MagicMock(
            pydantic=MagicMock(
                model_dump=lambda: {"recommendations": [], "reasoning_summary": "ok"}
            ),
            json_dict=None,
            raw="",
        )
    ...
    mock_crew.kickoff_async.assert_called_once_with()


@patch(f"{MODULE}.build_clinical_reasoning_crew")
@patch(f"{MODULE}.LLM")
async def test_reason_prefers_pydantic_output(mock_llm_cls, mock_build_crew):
    mock_build_crew.return_value.kickoff_async = AsyncMock(
        return_value=MagicMock(
            pydantic=MagicMock(
...
@patch(f"{MODULE}.build_clinical_reasoning_crew")
@patch(f"{MODULE}.LLM")
async def test_reason_falls_back_to_json_dict_when_no_pydantic(mock_llm_cls, mock_build_crew):
    mock_build_crew.return_value.kickoff_async = AsyncMock(
        return_value=MagicMock(
            pydantic=None,
            json_dict={"recommendations": [], "reasoning_summary": "from json_dict"},
...
@patch(f"{MODULE}.build_clinical_reasoning_crew")
@patch(f"{MODULE}.LLM")
async def test_reason_falls_back_to_parsing_raw_text_as_json(mock_llm_cls, mock_build_crew):
    mock_build_crew.return_value.kickoff_async = AsyncMock(
        return_value=MagicMock(
            pydantic=None,
            json_dict=None,
...
@patch(f"{MODULE}.build_clinical_reasoning_crew")
@patch(f"{MODULE}.LLM")
async def test_reason_raises_value_error_when_raw_is_not_valid_json(mock_llm_cls, mock_build_crew):
    mock_build_crew.return_value.kickoff_async = AsyncMock(
        return_value=MagicMock(
            pydantic=None,
            json_dict=None,
...
@patch(f"{MODULE}.build_clinical_reasoning_crew")
@patch(f"{MODULE}.LLM")
async def test_reason_raises_value_error_when_raw_json_is_not_an_object(
    mock_llm_cls, mock_build_crew
):
    mock_build_crew.return_value.kickoff_async = AsyncMock(
        return_value=MagicMock(
            pydantic=None,
